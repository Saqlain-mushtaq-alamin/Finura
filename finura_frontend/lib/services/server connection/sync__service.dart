...
class SyncService {
  static Future<void> syncAll() async {
    ...
    // 🔄 Sync notes
    final notes = await db.query('note_entry');
    for (var note in notes) {
      final success = await _postToAPI('/sync_note', note);
      if (success) {
        // assuming notes always overwrite on sync
        print("✅ Synced note: ${note['title']}");
      }
    }
  ...
  static Future<bool> _postToAPI(
    ...
    final baseUrl = 'http://your-fastapi-url.com'; // 🔁 Replace this
    try {
